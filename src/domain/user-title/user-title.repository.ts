...
export class UserTitleRepository {
  ...
  async findByUserIdAndSelected(
    userId: number,
    selected: boolean,
  ): Promise<UserTitle> {
    const selectedUserTitle: UserTitle = await this.repository.findOne({
      where: { user: { id: userId }, isSelected: selected },
    });
  ...
  async findByUserIdAndTitleId(
    userId: number,
    titleId: number,
  ): Promise<UserTitle> {
    const userTitle: UserTitle = await this.repository.findOne({
